 ...
 * change how generics are handled when searching facilities.
 *
 * <p>Additionally, by setting the {@code localPriority} flag, the search
 ...
 * facilities will still be searched if the facility strategy requires it.
 */
public class UnqualifiedPath implements ScopeSearchPath {
    ...
    private <E extends Symbol> boolean searchModule(
            ...
            Map<String, PTType> genericInstantiations,
            FacilitySymbol instantiatingFacility, ImportStrategy importStrategy,
        ...
        if (!finished && facilityStrategy != FacilityStrategy.FACILITY_IGNORE) {
            throw new UnsupportedOperationException("unqualified facility "
        ...
        if ((results.isEmpty() || !localPriority)
                && source instanceof BaseScope
                && importStrategy != ImportStrategy.IMPORT_NONE) {

            BaseScope sourceAsSyntacticScope = (BaseScope) source;
            ...
            try {
                ...
                for (String s : module.getImports()) {
                    finished =
                            searchModule(searcher, repo.getModuleScope(s), repo,
                                    results,
