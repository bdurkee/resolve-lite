...
/*
    ...
    private final RESOLVECompiler m_environment;
    private final ModuleScopeBuilder m_scope;
    ...
    public CongruenceClassProver(DumbMathClssftnHandler g,
                                 List<VC> vcs,
        ...
        List<TheoremSymbol> theoremSyms = new ArrayList<>();
        List<Symbol> zSyms = new ArrayList<>();

        try {
            theoremSyms.addAll(scope.query(new SymbolTypeQuery<TheoremSymbol>(TheoremSymbol.class,
                    MathSymbolTable.ImportStrategy.IMPORT_RECURSIVE,
                    MathSymbolTable.FacilityStrategy.FACILITY_IGNORE)));
            zSyms.addAll(scope.query(new NameQuery(null, "Z",
                            MathSymbolTable.ImportStrategy.IMPORT_RECURSIVE,
        ...
        MathClassification z = null;
        if (zSyms.size() > 0) {
            MathClssftnWrappingSymbol z_e = (MathClssftnWrappingSymbol) zSyms.get(0);
            z = z_e.getClassification();
        }

        List<Symbol> n_entries =
                null;
        try {
            n_entries = scope.query(new NameQuery(null, "N",
                    MathSymbolTable.ImportStrategy.IMPORT_RECURSIVE,
        ...
        MathClassification n = null;
        if (n_entries != null && n_entries.size() > 0) {
            MathClssftnWrappingSymbol n_e = (MathClssftnWrappingSymbol) n_entries.get(0);
            n = n_e.getClassification();
        }
        ...
        for (TheoremSymbol e : theoremSyms) {
            PExp assertion = Utilities.replacePExp(e.getAssertion(), m_typeGraph, z, n);
            String eName = e.getName();
            if (assertion.getTopLevelOperationName().equals("=B")
