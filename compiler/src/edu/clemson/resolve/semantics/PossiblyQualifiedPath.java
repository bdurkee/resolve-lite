...
/**
 * Like the name suggests, represents an implementation {@link ScopeSearchPath} that might (or might not be) qualified.
 * <p>
 * The implementation of {@link #searchFromContext(TableSearcher, Scope, MathSymbolTable)} here abstracts which of the
 * two are selected based on the parameters passed to {@code this}.</p>
 */
public class PossiblyQualifiedPath implements ScopeSearchPath {
    ...
    @NotNull
    private static ScopeSearchPath getAppropriatePath(@Nullable Token qualifier,
                                                      @NotNull ImportStrategy importStrategy,
                                                      @NotNull FacilityStrategy facilityStrategy,
                                                      boolean localPriority) {
